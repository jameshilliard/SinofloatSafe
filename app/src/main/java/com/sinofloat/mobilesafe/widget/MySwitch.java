...
public class MySwitch extends CompoundButton {
	...
	public MySwitch(Context context, AttributeSet attrs, int defStyle) {
		...
		mTrackDrawable = a.getDrawable(R.styleable.MySwitch_mytrack);
	 ...
	 * Typeface that you provided does not have replay_picture_all the bits in the style that
	 * you specified.
	 */
	 ...
	 * Note that not replay_picture_all Typeface families actually have bold and italic
	...
	private void stopDrag(MotionEvent ev) {
		...
		if (commitChange) {
			...
			animateThumbToCheckedState(isChecked());
			if (fixed) {//FIXME 这个是后加的逻辑
				boolean complete;
				if (mOrientation == HORIZONTAL) {
					float x = ev.getX();

					if (x - getLeft() >= ((2 / (float) 3) * getWidth())
							|| x >= getRight())
						complete = true;
					else
						complete = false;
					...
					if (y - getTop() >= ((2 / (float) 3) * getHeight())
							|| y >= getBottom())
						complete = true;
					else
						complete = false;
				...
				if (this.mOnChangeAttemptListener != null){
					this.mOnChangeAttemptListener.onChangeAttempted(complete);
				}
			}
		}
